...
export const CoordinatorDashboard: React.FC<CoordinatorDashboardProps> = ({ activeTab, setActiveTab }) => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const res = await api.courses.listAssigned();
      if (res.versions && res.versions.length > 0) {
        setAssignedVersions(res.versions);
  ...
  const saveDraftAutomatically = async (updatedVersion: any) => {
    ...
    try {
      const coordinatorPayload = {
        ...
        coPoMappings: updatedVersion.coPoMappings || [],
        coPsoMappings: updatedVersion.coPsoMappings || [],
        syllabusUnits: updatedVersion.syllabusUnits || [],
        labPracticals: [],
        miniProjects: [],
        textbooks: updatedVersion.textbooks || [],
        referenceMaterials: updatedVersion.referenceMaterials || [],
        journals: updatedVersion.journals || [],
        onlineResources: updatedVersion.onlineResources || [],
        objectives: updatedVersion.objectives || [],
  ...
  const handleSubmitToHod = async () => {
    ...
    try {
      ...
      await api.courses.saveDraft(activeVersion._id, coordinatorPayload);

      // 2. Update status to Pending HOD
      await api.courses.updateStatus(activeVersion._id, {
        status: 'Pending HOD',
        comments: 'Ready for Review. Drafted according to NBA guidelines.'
      });
      alert('Syllabus file successfully saved and submitted to HOD! Status is now Pending HOD.');
      loadCourseVersion(activeVersion._id);
      loadData();
    } catch (err: any) {
  ...
  const handleSaveReferenceMaterial = () => {
    ...
    const updatedVersion = { ...activeVersion, textbooks, referenceMaterials };
    setActiveVersion(updatedVersion);
    saveDraftAutomatically(updatedVersion);
    setRefForm({
      show: false,
  ...
  const handleAddOnlineResource = () => {
    ...
    const updatedVersion = { ...activeVersion, onlineResources };
    setActiveVersion(updatedVersion);
  ...
  const isLocked = activeVersion?.status === 'Pending HOD' || activeVersion?.status === 'Approved';
    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="grid grid-cols-12 gap-6 items-start">
        ...
        <div className={`transition-all duration-300 ${
            ...
            <div className="space-y-6 animate-fadeIn">
              ...
              <div className="bg-white rounded-2xl border border-border shadow-card p-6 flex flex-col sm:flex-row items-start gap-5">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[10px] font-semibold text-text-subtle uppercase tracking-widest">Aditya University · Coordinator Portal</p>
                  <h1 className="text-xl font-bold text-text-primary mt-0.5">Welcome back, {user?.name || 'Mr. N. Ramanjaneyulu'}</h1>
                  <div className="flex flex-wrap items-center gap-2 mt-3">
                    <span className="inline-flex items-center px-2.5 py-1 rounded-full bg-primary-50 text-primary-700 border border-primary-100 text-[11px] font-semibold">Course Coordinator</span>
                  ...
                  <p className="text-sm text-text-muted mt-3 leading-relaxed">
                    Currently assigned to coordinate curriculum definitions, outcome mapping matrices, and syllabus details for {assignedVersions.length} courses.
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-5 gap-4">
                {[
                  { label: 'Assigned Courses', sub: 'Syllabus definitions',        value: assignedVersions.length,                                                                          icon: BookOpen,      bg: 'bg-blue-50',    iconCl: 'text-blue-600',    border: 'border-blue-100' },
                  { label: 'Draft',            sub: 'Work in progress',             value: assignedVersions.filter(v => v.status === 'Draft').length,                                        icon: Edit,          bg: 'bg-slate-50',   iconCl: 'text-slate-600',   border: 'border-slate-200' },
                  { label: 'Submitted',        sub: 'Awaiting review',              value: assignedVersions.filter(v => v.status === 'Pending HOD' || v.status === 'Pending Admin').length,  icon: Send,          bg: 'bg-indigo-50',  iconCl: 'text-indigo-600',  border: 'border-indigo-100' },
                  { label: 'Approved',         sub: 'Accreditation ready',          value: assignedVersions.filter(v => v.status === 'Approved').length,                                     icon: CheckCircle2,  bg: 'bg-success-50', iconCl: 'text-success-600', border: 'border-success-100' },
                  { label: 'Returned',         sub: 'Requires corrections',         value: assignedVersions.filter(v => v.status === 'Returned').length,                                     icon: AlertTriangle, bg: 'bg-danger-50',  iconCl: 'text-danger-600',  border: 'border-danger-100' },
                ].map((kpi, idx) => {
                  ...
                  return (
                    <div key={idx} className="bg-white rounded-2xl border border-border shadow-card p-5 flex flex-col gap-3 hover:shadow-card-md hover:border-border-medium transition-all">
                      <div className={`w-9 h-9 rounded-xl flex items-center justify-center border ${kpi.bg} ${kpi.border}`}>
                        <Icon className={`w-4 h-4 ${kpi.iconCl}`} />
                      </div>
                      <div>
              ...
              <div className="grid grid-cols-3 gap-6">
                ...
                <div className="col-span-2 bg-white p-6 rounded-2xl border border-border shadow-card space-y-4">
                  <h3 className="text-xs font-semibold uppercase tracking-widest text-text-subtle border-b border-border pb-3">
                  ...
                  {activeVersion ? (
                    <div className="space-y-4">
                      {[
                        { label: 'Course Outcome (CO) Completion', val: getCOProgressPercent(), desc: `${getCOCount()} of 5 required COs defined` },
                        { label: 'CO-PO / CO-PSO Mapping Matrix', val: getMappingPercent(), desc: 'Percentage of non-zero mapping weights' },
                        { label: 'Syllabus Units Draft Compliance', val: getSyllabusPercent(), desc: `${activeVersion.syllabusUnits?.length || 0} of 5 units fully drafted` },
                        { label: 'Syllabus Submission Readiness', val: getReadinessPercent(), desc: 'Overall compilation score' }
                      ].map((bar, i) => (
                        <div key={i} className="space-y-1.5">
                          <div className="flex justify-between items-center text-xs">
                            <span className="font-semibold text-text-secondary">{bar.label}</span>
                            <span className="text-primary-700 bg-primary-50 px-2 py-0.5 rounded-full text-[10px] font-bold border border-primary-100">{bar.val}%</span>
                          </div>
                          <p className="text-[10px] text-text-subtle leading-none">{bar.desc}</p>
                          <div className="obe-progress-track">
                            <div className="obe-progress-fill bg-primary-600" style={{ width: `${bar.val}%` }}></div>
                          </div>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <p className="text-xs text-text-subtle text-center py-10">Select an active course version to display completion progress.</p>
                  )}
                </div>

                {/* Pending Tasks Panel */}
                <div className="bg-white p-6 rounded-2xl border border-border shadow-card flex flex-col justify-between">
                  <div>
                    <h3 className="text-xs font-semibold uppercase tracking-widest text-text-subtle border-b border-border pb-3">Pending Tasks</h3>
                    <div className="space-y-3 mt-4">
                      {[
                        { text: 'DBMS mapping matrix pending weights', active: getMappingPercent() < 50 },
                        { text: 'DBMS 5 Units details completion', active: getSyllabusPercent() < 100 },
                        { text: 'Syllabus submission to HOD review', active: activeVersion?.status === 'Draft' }
                      ].map((task, idx) => (
                        <div key={idx} className="flex items-start gap-2.5 text-xs">
                          <input
                            ...
                            checked={!task.active}
                            disabled
                            className="mt-0.5 rounded border-border-medium text-primary-600 focus:ring-primary-500"
                          />
                          <span className={`${!task.active ? 'line-through text-text-subtle' : 'text-text-secondary font-medium'}`}>{task.text}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  {activeVersion && (
                    <button
                      ...
                      className="w-full py-2.5 bg-primary-600 hover:bg-primary-700 text-white rounded-xl text-xs font-semibold cursor-pointer text-center transition-all mt-4 shadow-sm"
                    >
              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
                <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 border-b border-slate-100 pb-3 flex items-center gap-1.5">
                  ...
                  <span>Recent syllabus modifications feed logs</span>
                </h3>
                <div className="relative border-l border-slate-200 pl-4 ml-2 space-y-4 text-xs">
                  {[
                    { title: 'CO mapping updated', details: 'DBMS mapping matrix values updated.', time: '10:30 AM - 6/3/2026' },
                    { title: 'Syllabus modified', details: 'Unit 3 relational model description updated.', time: '09:15 AM - 6/2/2026' },
                    { title: 'Syllabus Returned by HOD', details: 'Returned with remarks: Improve CO3 bloom alignment.', time: '4:41 PM - 5/26/2026' }
                  ].map((act, i) => (
                    <div key={i} className="relative">
                      <div className="absolute -left-6 top-1 w-2.5 h-2.5 rounded-full bg-blue-600 border border-white"></div>
          ...
          {/* ========================================================== */}
          {/* TAB 2: MY ASSIGNED COURSES */}
          {/* ========================================================== */}
          {activeTab === 'my-courses' && (
            <div className="space-y-6 animate-fadeIn">
              <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                <h1 className="text-xl font-extrabold text-slate-800 font-sans">My Assigned Courses</h1>
                <p className="text-xs text-slate-500 mt-1"> Central repository bank of courses assigned to your account for syllabus mappings.</p>
              </div>

              {/* Filters Row */}
              <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-wrap gap-3 text-xs font-bold text-slate-500">
                <div className="relative flex-1 max-w-xs">
                  <input 
                    ...
                    className="w-full border border-slate-300 rounded-lg pl-8 pr-3 py-2 text-xs outline-none bg-white font-semibold focus:ring-1 focus:ring-blue-600 focus:border-blue-600"
                  />
                ...
                <select 
                  ...
                  className="border border-slate-300 rounded-lg p-2 bg-white outline-none w-32 font-semibold text-slate-650"
                >
                ...
                >
                  ...
                  {Array.from(new Set(assignedVersions.map(v => v.regulationId?.code).filter(Boolean))).map(code => (
              ...
              <div className="grid grid-cols-2 gap-6">
                {assignedVersions
                  .filter(v => {
                    const matchesSearch = v.courseId?.code.toLowerCase().includes(courseSearch.toLowerCase()) || v.courseId?.title.toLowerCase().includes(courseSearch.toLowerCase());
                    const matchesSem = filterSemester ? v.semester.toString() === filterSemester : true;
                    const matchesStatus = filterStatus ? v.status === filterStatus : true;
                    const matchesReg = filterRegulation ? v.regulationId?.code === filterRegulation : true;
                    return matchesSearch && matchesSem && matchesStatus && matchesReg;
                  })
                  .map((v) => (
                    <div key={v._id} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between h-[220px] hover:shadow-premium transition-all">
                      <div>
                        <div className="flex justify-between items-start">
                          <span className="text-[10px] font-mono text-slate-400 font-bold uppercase tracking-wide">Semester {v.semester} • {v.regulationId?.code}</span>
                          <span className={`px-2 py-0.5 rounded text-[9px] font-bold border ${
                            v.status === 'Approved' ? 'bg-emerald-50 text-emerald-700 border-emerald-100 font-bold' :
                            v.status === 'Pending HOD' ? 'bg-indigo-50 text-indigo-700 border-indigo-100' :
                            v.status === 'Returned' ? 'bg-red-50 text-red-700 border-red-100 animate-pulse' :
                            'bg-slate-100 text-slate-600 border-slate-200'
                          }`}>
                            {v.status}
                          </span>
                        </div>
                        <h3 className="text-base font-extrabold text-blue-800 mt-2.5 leading-snug">{v.courseId?.code} — {v.courseId?.title}</h3>
                        <p className="text-xs text-slate-400 mt-1 font-semibold">{v.courseId?.departmentId?.name || 'Computer Science Engineering'}</p>
                        
                        <div className="flex items-center gap-4 mt-3.5 text-[10px] font-bold text-slate-500 font-mono">
                          <span>Credits: {v.credits?.C || 4}</span>
                          <span>L-T-P-S: {v.credits?.L || 3}-{v.credits?.T || 1}-{v.credits?.P || 0}-{v.credits?.S || 0}</span>
                        </div>
                      </div>
                      ...
                      <div className="border-t border-slate-100 pt-4 flex justify-between items-center mt-4">
                        <button 
                          ...
                          className="py-2 px-4 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-[10px] font-extrabold uppercase transition-all shadow cursor-pointer"
                        >
                          Continue Editing Workspace
                        </button>
                        {v.comments && (
                          <button 
                            onClick={() => setSelectedRemarks(v.comments)}
                            className="text-[10px] text-red-600 underline font-bold"
                          >
                            View Remarks
                          </button>
          ...
          {/* ========================================================== */}
          {/* SYLLABUS EDITING WORKSPACE (WRAPPED IN LOCKING FIELDSET) */}
          {/* ========================================================== */}
          {activeTab !== 'dashboard' && activeTab !== 'my-courses' && activeTab !== 'profile' && activeTab !== 'reports' && activeTab !== 'work-progress' && activeTab !== 'notifications' && (
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-6 animate-fadeIn">
              ...
              <div className="flex justify-end pt-4 mt-6 border-t border-slate-100">
                <button
                  ...
                  className="flex items-center gap-2 px-6 py-2.5 bg-blue-600 text-white rounded-xl text-xs font-bold hover:bg-blue-700 transition-all shadow-sm"
                >
                  <span>Next: Course Outcomes</span>
          ...
          {activeTab === 'cos' && activeVersion && (
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-6 animate-fadeIn">
              ...
              <div className="flex justify-between pt-4 mt-6 border-t border-slate-100">
                <button
                  onClick={() => setActiveTab('course-details')}
                  className="flex items-center gap-2 px-6 py-2.5 bg-slate-100 text-slate-600 rounded-xl text-xs font-bold hover:bg-slate-200 transition-all shadow-sm"
                >
          ...
          {activeTab === 'co-po' && activeVersion && (
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-5 animate-fadeIn">
              ...
              <div className="flex justify-between pt-4 mt-6 border-t border-slate-100">
                ...
                >
                  <ArrowLeft className="w-4 h-4" />
                  <span>Back: Course Outcomes</span>
                </button>
                <button
                  onClick={() => setActiveTab('co-pso')}
                  className="flex items-center gap-2 px-6 py-2.5 bg-blue-600 text-white rounded-xl text-xs font-bold hover:bg-blue-700 transition-all shadow-sm"
                >
                  <span>Next: CO-PSO Mapping</span>
                  <ArrowRight className="w-4 h-4" />
                </button>
              </div>
            </div>
          ...
          {activeTab === 'co-pso' && activeVersion && (
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-5 animate-fadeIn">
              ...
              <div className="flex justify-between pt-4 mt-6 border-t border-slate-100">
                ...
                >
                  <ArrowLeft className="w-4 h-4" />
                  <span>Back: CO-PO Mapping</span>
                </button>
                <button
                  onClick={() => setActiveTab('syllabus')}
                  className="flex items-center gap-2 px-6 py-2.5 bg-blue-600 text-white rounded-xl text-xs font-bold hover:bg-blue-700 transition-all shadow-sm"
                >
                  <span>Next: Syllabus Management</span>
                  <ArrowRight className="w-4 h-4" />
                </button>
              </div>
            </div>
          ...
          {activeTab === 'syllabus' && activeVersion && (
            <div className="space-y-6 animate-fadeIn">
              ...
              <div className="flex justify-between pt-4 mt-6 border-t border-slate-100">
                ...
                >
                  <ArrowLeft className="w-4 h-4" />
                  <span>Back: CO-PSO Mapping</span>
                </button>
                <button
                  onClick={() => setActiveTab('reports')}
                  className="flex items-center gap-2 px-6 py-2.5 bg-blue-600 text-white rounded-xl text-xs font-bold hover:bg-blue-700 transition-all shadow-sm"
                >
                  <span>Next: Reports & Submission</span>
                  <ArrowRight className="w-4 h-4" />
                </button>
              </div>
          ...
          {activeTab === 'reports' && activeVersion && (
            <div className="space-y-6 animate-fadeIn">
                  ...
                  <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    {reportWarning ? (
                      ...
                      <div className="mt-5 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-xs text-emerald-700 animate-fadeIn">
                        <div className="font-bold flex items-center gap-1.5">
                          <span>🎉</span>
                          <span>Ready for export and HOD submission!</span>
                        </div>
                    ...
                    <div className="mt-5">
                      <button
                        ...
                        className={`w-full px-4 py-3 rounded-xl text-xs font-extrabold uppercase transition-all shadow-sm ${reportReady ? 'bg-blue-600 hover:bg-blue-700 text-white cursor-pointer' : 'bg-slate-100 text-slate-400 border border-slate-200 cursor-not-allowed shadow-none'}`}
                      >
                        Submit to HOD
                      </button>
                    </div>
              ...
              <div className="flex justify-start pt-4 mt-6 border-t border-slate-100">
                <button
                  onClick={() => setActiveTab('syllabus')}
                  className="flex items-center gap-2 px-6 py-2.5 bg-slate-100 text-slate-600 rounded-xl text-xs font-bold hover:bg-slate-200 transition-all shadow-sm"
                >
                  <ArrowLeft className="w-4 h-4" />
                  <span>Back: Syllabus Management</span>
                </button>
              </div>
